...
`run_ai_mode_sync(run_id)` is a synchronous, **never-raises** 3-phase pipeline (errors land in `status.json`, `status="failed"`), scheduled from the router via `asyncio.to_thread`:
...
2. **Clean** — per-batch LLM cleanup. Sync (`AI_MODE_LLM_BATCH=false`) via `make_llm_client` (Gemini *or* OpenAI-compatible), or the **Gemini Batch API** (`=true`, forces the Gemini provider) sharded/polled in `gemini_batch.py`. Results map back **by key**, never by position.
3. **Assemble** — merges rerun `carryover.json`, computes cost, writes outputs.

`status.json` is persisted after every batch for UI polling. Modes are defined in `mode_config.py` (`ai_bulk`=batch 10 / `ai_bulk_search.txt`; `ai_deep`=batch 3 / `ai_deep_search.txt`; both share `ai_cleanup.txt`). Provider/keys: `build_ai_mode_llm_config` reads `AI_MODE_LLM_PROVIDER` (`gemini`|`openai`, `OPENAI_BASE_URL` for gateways); validated at **upload time** (`prepare_ai_mode_run`, 400 on missing key) and again at run time (`SCRAPEDO_TOKEN` required). Rerun (`rerun.py`) re-runs only failed/unscraped rows and merges prior successes via `carryover.json` — it re-enters the same `run_ai_mode_sync`.
...
- AI Mode: `ai_mode_results/<company_slug>/<run_id>/` — `input.csv`, `status.json`, `final_report.json`, `found.csv`, `notFound.csv`, `run.log`, `raw_responses/request_NNN.json`, `carryover.json` (reruns). Legacy read-only dir: `ai_mode_result/` (singular).
