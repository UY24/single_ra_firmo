...
- **`official_website` is an INPUT here, so it cannot mean "found"** (fixed 2026-08-19).
  ...
  `serpwow_reporting.row_produced_a_result(result, pipeline)`, called from BOTH
...
- **No SerpWow left on this path** (2026-08-19): `codetails.py` deleted,
  ...
  **`search_response/…_search.json`** via `engine._raw_artifact_names(pipeline)` — a
  ...
  `services/serpwow/` **package path**, the shared `serpwow_reporting` module, and the
  `CrawlResponse.serpwow_cost_usd` / `s3_serpwow_json_key` fields + their CSV columns —
  those are cross-pipeline schema, and the package rename is the agreed last step.
