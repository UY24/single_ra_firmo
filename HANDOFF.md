...
Last updated: 2026-07-08 (NEW `relationship` pipeline, committed on branch `relationshipMode`). Read this first if you're picking up this repo.

---

## ⭐ 2026-07-08 — NEW `relationship` pipeline (branch `relationshipMode`, 10 commits over `79b5ec5`, NOT pushed/merged)
...
**Live-verified 2026-07-08** (rel.csv = 10 rows: 5 blank, 5 noisy pairs; company "test isi market"): BOTH modes end-to-end through RabbitMQ. Per-row run `1fa46851…`: OCR-garbage "YUZU…SANZO…" → `drinksanzo.com` confirmed conf 95; junk pairs correctly gated (`not_confirmed`/`unclear`, URLs preserved in flags); a transient Gemini 503 exercised the retryable row-error path. Batch run `e5a832a0…`: batch `waiting_for_rows→running→succeeded`, Modal → `modal.com` confirmed 95, zero-evidence short-circuit inside batch mode confirmed (row failed pre-seed, no tokens). S3 layout, `skipped.csv` endpoint (`?file=skipped.csv`), summary counts/breakdown/cost/tokens all verified. Suite **346/346** green (`-t .`).

**Known minors (accepted, logged in `.superpowers/sdd/progress.md`):** stale gsearch/gmaps-only wording in a few engine comments; `x_domain_from_input_url` returns "" for scheme-less URLs; UI `countCsvRows` preview count ignores CSV quoting (cosmetic); relationship rows in a *failed* batch chunk keep the stale "Pending Gemini batch post-processing decision." error text; Gemini batch job display_name still says `gsearch-…` for relationship chunks (cosmetic). LLM picks for near-name collisions (e.g. "ＫＡＲＭIＣ" → different Karmic sites across runs, both "confirmed" 95) are a data-quality reality of noisy OCR + search variance — a candidate/summary review pass is the mitigation if it matters.
